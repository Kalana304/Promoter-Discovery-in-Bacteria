...
//
// Kalana Abeywardena, ENTC, University of Moratuwa (160005C@uom.lk) 2020/07/02
// -------------------------------------------------------------------------------
...
function [inserts, mutations]=pribnow_mutate(x_seq, y_seq)
    subs = 0;
    prib_box = 6;
    col_num = 1:length(y_seq);
    bases = col_num(y_seq ~= ascii('-'));
    inserts = length(y_seq) - length(bases);
    aligned_x = x_seq(1,bases);
    l_seq = length(aligned_x);
    
    if l_seq == prib_box then
        for i=1:l_seq
            if (aligned_x(i)==ascii('G'))|(aligned_x(i)==ascii('C')) then
                subs = subs + 1;
            end
        end
    end
...
function [index]=base2index(base)
    if (base == ascii('A')) then
        index = 1;
    elseif (base == ascii('C')) then
        index = 2;
    elseif (base == ascii('G')) then
        index = 3; 
    else
        index = 4;
    end
...
function [pos_mut, point_mut, inserts, mutations]=sigma_mutate(x_seq, y_seq)
    subs = 0;
    sig_bind = 6;
    pos_mat = zeros(1,sig_bind);
    point_mat = zeros(1,2);
    
    col_num = 1:length(y_seq);
    bases = col_num(y_seq ~= ascii('-')); //disp(bases)
    inserts = length(y_seq) - length(bases);
    aligned_x = x_seq(1,bases); //disp(ascii(aligned_x))
    l_seq = length(aligned_x);
    
    if l_seq == sig_bind then
        if (aligned_x(1)~=ascii('T')) then
            base_ind = base2index(aligned_x(1));
            pos_mat(1,1) = pos_mat(1,1) + 1;
            subs = subs + 1;
            if ((base_ind == 2)|(base_ind == 3)) then
                point_mat(1,2) = point_mat(1,2) + 1; 
            else
                point_mat(1,1) = point_mat(1,1) + 1;
            end
        end
        if (aligned_x(2)~=ascii('T')) then
            base_ind = base2index(aligned_x(2));
            pos_mat(1,2) = pos_mat(1,1) + 1;
            subs = subs + 1;
            if ((base_ind == 2)|(base_ind == 3)) then
                point_mat(1,2) = point_mat(1,2) + 1; 
            else
        ...
        if (aligned_x(3)~=ascii('G')) then
            base_ind = base2index(aligned_x(3));
            pos_mat(1,3) = pos_mat(1,1) + 1;
            subs = subs + 1;
            if ((base_ind == 2)|(base_ind == 3)) then
                point_mat(1,2) = point_mat(1,2) + 1; 
            else
        ...
        if (aligned_x(4)~=ascii('A')) then
            base_ind = base2index(aligned_x(4));
            pos_mat(1,4) = pos_mat(1,1) + 1;
            subs = subs + 1;
            if ((base_ind == 2)|(base_ind == 3)) then
                point_mat(1,2) = point_mat(1,2) + 1; 
            else
        ...
        if (aligned_x(5)~=ascii('C')) then
            base_ind = base2index(aligned_x(5));
            pos_mat(1,5) = pos_mat(1,1) + 1;
            subs = subs + 1;
            if ((base_ind == 2)|(base_ind == 3)) then
                point_mat(1,2) = point_mat(1,2) + 1; 
            else
        ...
        if (aligned_x(6)~=ascii('A')) then
            base_ind = base2index(aligned_x(6));
            pos_mat(1,6) = pos_mat(1,1) + 1;
            subs = subs + 1;
            if ((base_ind == 2)|(base_ind == 3)) then
                point_mat(1,2) = point_mat(1,2) + 1; 
            else
